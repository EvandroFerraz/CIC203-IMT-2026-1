...
public class NossoVetor {
    ...
    private int partition(int p, int r){
        int x = vetor[r]; // pivô = ultima posição do vetor
        int i = p-1; // i = indice inicial - 1
        int aux;
        for(int j = p; j < r; j++){ // vai até chegar no pivô
            if(vetor[j] <= x){
                i = i + 1;
                aux = vetor[i];
                vetor[i] = vetor[j];
        ...
        } // quando sairmos do laço i = q-1, ou seja, q = i + 1
        i = i + 1; // i passa a ser o índice do meio
        aux = vetor[r]; // auxiliar guarda o pivô
        vetor[r] = vetor[i]; // meio vai para ultima posicao
        vetor[i] = aux; // meio recebe o pivô
        return i;
    } 

    // quicksort() para ordenação
    // p = indice do inicio do arranjo
    // r = indice final do arranjo
    public void quickSort(int p, int r){
        if (p < r){ // caso base: p >= r, quando já percorremos todas posições
            int q = partition(p, r);
            quickSort(p, q-1); // metade esquerda, numeros menores que o pivô
            quickSort(q+1, r); // metade direita, numermos maiores que o pivô
        }
    } 
}